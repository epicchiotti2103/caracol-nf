...
export function ClientEditModal({ client, onClose, onSaved }: Props) {
  ...
  const [entity, setEntity] = useState<ClientEntity>(client?.default_entity ?? "BR");
  const [moeda, setMoeda] = useState<Moeda>(client?.default_moeda ?? "BRL");
  ...
  const buildPayload = (): ClientCreatePayload | ClientUpdatePayload => {
    const out: any = {
      name: name.trim(),
