...
- `GET /nf/link-suggestions?month=YYYY-MM` — NFs a pagar sem campanha vinculada + campanhas candidatas. Retorna `{month, suggestions: [{invoice_id, number, supplier_id, supplier_name, amount, currency, competencias, candidates: [{campaign_id, campaign_name, month, publisher_amount}], candidates_total, match: "exato"|"aproximado"|"divergente"}]}`. `POST /nf/link-suggestions/apply {invoice_id, campaign_ids}` -> `{invoice_id, linked_campaign_ids}`. Consumido por `/sugestoes-vinculo` e pelo banner da lista a pagar (`lib/link-suggestions.ts`; 404/405 = rota nao publicada -> degrada).
