...
- `GET /nf/link-suggestions?month=YYYY-MM` — NFs a pagar sem campanha vinculada + campanhas candidatas. Retorna `{month, suggestions: [{invoice_id, number, supplier_id, supplier_name, status, amount, amount_month, currency, due_date, competencias, already_linked_campaign_ids, candidates: [{campaign_id, codigo, campaign_name, month, publisher_amount, fechamento_status: "travado"|"fechado"}], candidates_total, diff, match: "exato"|"aproximado"|"divergente"}]}`. `match`/`diff` comparam `candidates_total` com `amount_month` (valor da competencia do mes; front usa `amount` se ausente). `POST /nf/link-suggestions/apply {invoice_id, campaign_ids}` -> `{invoice_id, linked_campaign_ids, added}` (`added` = so os novos). Match divergente: candidatas desmarcadas por padrao + aviso. Consumido por `/sugestoes-vinculo` e pelo banner da lista a pagar (`lib/link-suggestions.ts`; 404/405 = rota nao publicada -> degrada).
...
- **Troca so da tag, qualquer status**: `PATCH /nf/invoices/{id}/tag` e `PATCH /nf/receivables/{id}/tag`, body `{tag_id: uuid|null}` → `{id, tag_id, tag_name}`. Vale em qualquer status (exceto deletada/cancelada), admin/adm_campanha. Consumido por `components/nf/tag-quick-edit.tsx` (detalhe da NF a pagar + lista de a receber). 404 = backend sem a rota → mensagem amigavel.
...
- [x] Marcar NF como paga aceita **comprovante opcional** (modal de upload PNG/JPEG/PDF max 10MB → multipart pra `POST /pay`; sem anexo o campo `proof` nao vai no FormData). Vale tanto pro pagamento individual quanto pro **lote** (`POST /nf/payment-batches` — 1 comprovante pra todas ou nenhuma, nunca subconjunto). Detalhe exibe botao "Baixar comprovante" so quando `paid_proof_path` esta setado (consome `GET /nf/invoices/{id}/proof`). **Simetrico ao lado a receber** (`POST /nf/receivables/{id}/receive`, `proof` opcional; listagem so mostra o botao quando `received_proof_path` esta setado). **O PDF da nota fiscal no cadastro continua obrigatorio** — a flexibilizacao vale so pro comprovante de pagamento/recebimento
