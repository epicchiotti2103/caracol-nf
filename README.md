...
- `/sugestoes-vinculo` — **Sugestoes de vinculo** NF a pagar -> campanha (admin/adm_campanha; link "Sugestoes" no menu). Seletor de mes (default mes anterior; deep-link `?month=YYYY-MM`, usado pelo Gerencial). Por NF sem campanha: fornecedor, numero, valor, competencias, candidatas com checkbox (todas marcadas; em match divergente comecam desmarcadas com aviso), soma marcadas vs `amount_month` (valor da competencia do mes; mostra tambem o total da NF quando difere) e badge do match (exato/aproximado/divergente). "Vincular" chama o apply e tira a NF da lista — nada automatico. Rota 404 no backend -> estado "recurso ainda nao disponivel". A lista de NF a pagar mostra o aviso "X NFs com sugestao de vinculo" (`components/nf/link-suggestions-banner.tsx`, silencioso em erro/404).
...
- **Sugestoes do fechamento** (so NF a pagar, quando ha **fornecedor escolhido**) — ao selecionar o fornecedor, o bloco busca `GET /nf/fechamento-sugestoes?supplier_id=<uuid>` → `{items:[{mes_referencia, campanha_id, campanha_name, codigo?, valor_sugerido, moeda}]}` e mostra as campanhas onde esse fornecedor tem pagamento **a receber do fechamento** e que ainda **nao tem NF a pagar vinculada**, **agrupadas por mes de referencia**. Cada sugestao e um **checkbox + valor sugerido** (bruto do fechamento, **editavel** apos marcar). **Nao auto-marca nada** — o usuario escolhe quais campanhas a NF cobre (multi-mes). Marcar cria um vinculo `{campanha_id, valor}`; desmarcar remove. Ao **trocar de fornecedor**, os vinculos vindos de sugestao sao descartados (preservando os adicionados manualmente). `codigo` e lido defensivamente (cai em `campanha_name` se ausente). Graceful: 404/erro suprime o bloco sem quebrar o form. Bloco invisivel na NF a receber e quando nao ha fornecedor (cai no modo manual).
- **Campanhas** (busca manual / "Outras campanhas") — lista dinamica de linhas `{campanha, valor}` pra vincular campanhas **fora da sugestao**. Cada linha tem um **seletor de mes de referencia** (populado por `GET /campanhas?months_available=1` → `{months: ["YYYY-MM", ...]}` desc, cacheado no bloco e compartilhado entre as linhas; default "Todos os meses") + busca de campanha (`GET /campanhas?month=<YYYY-MM>&q=<texto>`, `month` e `q` combinaveis; mostra "CMP-NNN — nome — MM/YYYY") + input de valor com mascara monetaria. O mes filtra a busca pra a lista nao crescer demais. Botao "Adicionar campanha" e remover por linha. Mostra a **soma das alocacoes vs total da NF** (incluindo as de sugestao) — aviso visual discreto (ambar) quando diferente, **sem bloquear o submit**.
...
**Troca de tag em qualquer status** (`components/nf/tag-quick-edit.tsx`): admin/adm_campanha trocam SO a tag de NF ja aprovada/paga (a pagar, na linha "Tag" do detalhe `/invoice/[id]`) ou recebida/pendente (a receber, lapis ao lado do chip na lista; cancelada nao). Usa rotas dedicadas `PATCH /nf/invoices/{id}/tag` e `PATCH /nf/receivables/{id}/tag` (body `{tag_id|null}` → `{id, tag_id, tag_name}`); o resto da NF continua travado. 404 (backend sem a rota) vira mensagem amigavel.
...
Ao marcar como paga, o admin pode anexar o **comprovante de pagamento** (PNG/JPEG/PDF, max 10MB) — **opcional** desde 2026-07-28. Se anexar, o backend grava `paid_proof_path` e o detalhe ganha um botao "Baixar comprovante" que abre uma URL assinada (`GET /nf/invoices/{id}/proof`); sem anexo, o botao simplesmente nao aparece. **Nao confundir com o PDF da nota fiscal, que continua obrigatorio no cadastro** (`/invoice/new`).

### Pagamento em lote (1 transferencia, varias NFs)

Quando uma transferencia bancaria quita VARIAS NFs de uma vez (mesmo comprovante) e tem UMA taxa (~US$ 40 por transferencia, nao por nota), use o **"Pagar em lote"** (botao no topo da lista "A pagar", so admin). O modal (`components/nf/batch-pay-modal.tsx`) lista as NFs `aprovada` por moeda, aceita **1 comprovante compartilhado (opcional)** — ou todas as NFs do lote ficam com o mesmo `paid_proof_path`, ou nenhuma fica —, recebe a **taxa** (default 40) + data + seletor **"Saiu de qual conta?"** (opcoes por moeda, ver `lib/contas.ts`), e marca todas como pagas de uma vez.
