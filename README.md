...
- **Campanhas** — lista dinamica de linhas `{campanha, valor}`. Cada linha tem busca de campanha (`GET /campanhas?q=<texto>`, mostra "CMP-NNN — nome — MM/YYYY") + input de valor com mascara monetaria. Botao "Adicionar campanha" e remover por linha. Mostra a **soma das alocacoes vs total da NF** — aviso visual discreto (ambar) quando diferente, **sem bloquear o submit**.
